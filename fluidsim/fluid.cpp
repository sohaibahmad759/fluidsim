...
// 0 for negligible surface tension (free flow)
// 1 for moderate surface tension (water-like flow)
// 2 for high surface tension (drop formation)
int surfaceTension = 1;
...
Particle::Particle(double xV, double yV, double zV, int type, int st) {
	surfaceTension = st;
	if (type == 0) {
		xVel = xV;
		yVel = yV;
	...
	for (int i=0; i<pList.size(); i++) {
		...
		for (int j=0; j<pList.size(); j++) {
			...
			if (neighbourDistSqr <= closeSquared) {
				...
				int surfacetensionConstant;
				if (surfaceTension == 0) {
					restDensity = 0;
					surfacetensionConstant = 1;
				}
				else if (surfaceTension == 1) {
					surfacetensionConstant = 2;
				}
				else if (surfaceTension == 2) {
					surfacetensionConstant = 10;
				}
				double dm = q * (pi.press + neighbourpi.press - 2*restDensity)/100 + q2 * (pi.pressnear + neighbourpi.pressnear)/surfacetensionConstant;
				...
				double velDiffx = (neighbourpi.xVel-pi.xVel);
				double velDiffy = (neighbourpi.yVel-pi.yVel);
				double velDiffz = (neighbourpi.zVel-pi.zVel);

				double u = velDiffx*dirX + velDiffy*dirY + velDiffz*dirZ;

				if (u > 0) {
					double visc = (1.0 - q) * (u*sigma + u*u*beta);
					double viscX = visc * abs(neighbourpi.x-pi.x);
					double viscY = visc * abs(neighbourpi.y-pi.y);
					double viscZ = visc * abs(neighbourpi.z-pi.z);

					pi.xVel -= viscX;
					pi.yVel -= viscY;
					pi.zVel -= viscZ;

					neighbourpi.xVel += viscX;
					neighbourpi.yVel += viscY;
					neighbourpi.zVel += viscZ;
				}
